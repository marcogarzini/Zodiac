...
class Recipe(Scripts):
    ...
    def write_pkg_resources(self, ws, d):
        ...
        if pkg_resources_loc:
            shutil.copyfile(os.path.join(pkg_resources_loc, pkg_resources),
                            os.path.join(d, pkg_resources))
        else:
            ...
            if os.path.isdir(setuptools_eggs[0]):
                src = os.path.join(setuptools_eggs[0], pkg_resources)
                dst = os.path.join(d, pkg_resources)
                shutil.copyfile(src, dst)
            else:
                egg_contents = zipfile.ZipFile(setuptools_eggs[0])
                pkg_resources = open(os.path.join(d, pkg_resources), "w")
                pkg_resources.write(egg_contents.read(pkg_resources))
                pkg_resources.close()
